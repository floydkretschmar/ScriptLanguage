...
ergebnis A + B.";
    ...
    {
        Assert.That(() => _parser.Parse<double>(Lexer.Tokenize(script)),
            Throws.TypeOf<ParseException>().With.Message.Contains(messageSnippet));
    }
    ...
    {
        Assert.That(() => _parser.Parse<bool>(Lexer.Tokenize(script)),
            Throws.TypeOf<ParseException>().With.Message.Contains(messageSnippet));
    }
}