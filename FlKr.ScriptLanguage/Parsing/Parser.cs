...
internal delegate Expression ParseOperation(List<IToken> expression, ParsingContext context, out Type dataType);
...
public partial class Parser
{
    ...
    public Func<T> Parse<T>(List<IToken> tokens)
    {
        ...
        var resultVariable = Expression.Variable(typeof(T));
    ...
    private List<List<IToken>> SplitIntoEndOfLineExpressions(List<IToken> tokens)
    {
        ...
        for (var i = 0; i < endOfLineExpressions.Count; i++)
    ...
    private List<Expression> ParseStatements(List<IToken> tokens, ParsingContext context)
    {
        ...
        var expressionLambdas = new List<Expression>();
